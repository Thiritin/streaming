...
8. **Done.** Retired the whole MP4 path: `DvrExtractorService`, `dvr-extract.sh`,
   `dvr-process.sh`, `dvr-convert.sh`, `ExtractDvrSegments`, `SrsDvrController` and its
   two routes, `uploader.py`, the `dvr-uploader` service and its volume, and the SRS
   `dvr` block (now `enabled off`). `docker/dvr-uploader` became
   `docker/archive-uploader`, and `stream.images.dvr_uploader` became
   `stream.images.archive_uploader`.
...
That also settles the disk question `ARCHIVE_SOURCE` would otherwise have raised: the
origin now carries the ladder window and the source window, where before it carried the
ladder window and the MP4 chunks. See *1b. Origin: the source rendition* for the sizing.

Deleted alongside the above, as stale rather than as part of this plan: repo-root
