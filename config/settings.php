    ...
    */
    ...
    'groups' => [
        ...
        [
            ...
            'icon' => 'server',
            'label' => 'Servers',
            'description' => 'Where a server is provisioned, and who writes the name it answers to.',
            'cards' => [
                [
                    'key' => 'cloud',
                    ...
                    'fields' => [
                        [
                            ...
                            'type' => 'select',
                            'store' => 'stream',
                            'config' => 'stream.server.provider',
                            'options' => [
                                'hetzner' => 'Hetzner Cloud',
                                'manual' => 'Bring your own server',
                            ],
                            'rules' => ['required', 'string', 'in:hetzner,manual'],
                        ],
                        [
                            'key' => 'hetzner_token',
                            'label' => 'API token',
                            'type' => 'password',
                            'config' => 'services.hetzner.token',
                            'secure' => true,
                            'full' => true,
                            'visible_when' => ['field' => 'cloud_driver', 'is' => ['hetzner']],
                            'rules' => ['required', 'string', 'max:255'],
                        ],
                        [
                        ...
                        [
                            ...
                            'type' => 'text',
                            'store' => 'stream',
                            'config' => 'stream.server.image',
                            'visible_when' => ['field' => 'cloud_driver', 'is' => ['hetzner']],
                            'rules' => ['nullable', 'string', 'max:64'],
                        ],
                        [
                            'key' => 'hetzner_ssh_key_name',
                            'label' => 'SSH key name',
                            'type' => 'text',
                            'store' => 'stream',
                            'config' => 'stream.server.ssh_key',
                            'helper' => 'As it is named in the Hetzner project. Empty provisions without one.',
                            'visible_when' => ['field' => 'cloud_driver', 'is' => ['hetzner']],
                            'rules' => ['nullable', 'string', 'max:255'],
                        ],
                        [
                            'key' => 'hetzner_network_name',
                            'label' => 'Private network name',
                            'type' => 'text',
                            'store' => 'stream',
                            'config' => 'stream.server.network',
                            'helper' => 'Empty provisions without one, and edges then reach the origin over the public address.',
                            'visible_when' => ['field' => 'cloud_driver', 'is' => ['hetzner']],
                            'rules' => ['nullable', 'string', 'max:255'],
                        ],
                        [
                            'key' => 'server_default_origin',
                            'label' => 'Default origin size',
                            'type' => 'text',
                            'store' => 'stream',
                            'config' => 'stream.server.defaults.origin',
                            'visible_when' => ['field' => 'cloud_driver', 'is' => ['hetzner']],
                            'rules' => ['nullable', 'string', 'max:32'],
                        ],
                        [
                            'key' => 'server_default_edge',
                            'label' => 'Default edge size',
                            'type' => 'text',
                            'store' => 'stream',
                            'config' => 'stream.server.defaults.edge',
                            'visible_when' => ['field' => 'cloud_driver', 'is' => ['hetzner']],
                            'rules' => ['nullable', 'string', 'max:32'],
                        ],
                    ],
                ...
                [
                    ...
                    'render' => 'dns_test',
                    'note' => [
                        'text' => 'The zone must resolve from the public internet. A server takes a certificate for its own hostname on first boot, and an unreachable name leaves it off 443 entirely.',
                    ],
                    'fields' => [
                        [
                            'key' => 'dns_driver',
                            ...
                            'config' => 'dns.driver',
                            'options' => [
                                'rfc2136' => 'nsupdate (RFC2136)',
                                'cloudflare' => 'Cloudflare',
                                'hetzner' => 'Hetzner DNS',
                                'none' => 'None - records are not written',
                            ],
                            'rules' => ['required', 'string', 'in:rfc2136,cloudflare,hetzner,none'],
                        ],
                        // Every driver needs the zone, and so do hostname composition and
                        // the origin fallback in the generated nginx config.
                        [
                            'key' => 'dns_zone',
                            'label' => 'Zone',
                            'type' => 'text',
                            'config' => 'dns.zone',
                            // A hostname and nothing else. It reaches nsupdate's script,
                            // so anything that could be read as a second command is
                            // refused here as well as at the point of use.
                            'rules' => ['nullable', 'string', 'max:255', 'regex:/^[A-Za-z0-9]([A-Za-z0-9.-]*[A-Za-z0-9])?$/'],
                        ],
                        [
                            'key' => 'dns_ttl',
                            'label' => 'TTL (seconds)',
                            ...
                            'config' => 'dns.ttl',
                            'cast' => 'int',
                            'rules' => ['nullable', 'integer', 'min:30', 'max:86400'],
                        ],
                        [
                            'key' => 'dns_server',
                            'label' => 'Name server',
                            ...
                            'store' => 'dns',
                            'config' => 'dns.server',
                            'helper' => 'The key must be authorised for the zone and this installation\'s address allowed to update it.',
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['rfc2136']],
                            'rules' => ['required', 'string', 'max:255', 'regex:/^[A-Za-z0-9]([A-Za-z0-9.:-]*[A-Za-z0-9])?$/'],
                        ],
                        [
                            'key' => 'dns_key_name',
                            'label' => 'TSIG key name',
                            ...
                            'store' => 'dns',
                            'config' => 'dns.key_name',
                            // Written into the key file the same way, so the same rule.
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['rfc2136']],
                            'rules' => ['required', 'string', 'max:255', 'regex:/^[A-Za-z0-9]([A-Za-z0-9._-]*[A-Za-z0-9])?$/'],
                        ],
                        [
                            'key' => 'dns_key_algorithm',
                            'label' => 'TSIG algorithm',
                            'type' => 'select',
                            'store' => 'dns',
                            'config' => 'dns.key_algorithm',
                            'options' => [
                            ...
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['rfc2136']],
                            'rules' => ['required', 'string', 'in:hmac-sha256,hmac-sha512,hmac-sha1,hmac-md5'],
                        ],
                        [
                            'key' => 'dns_key_secret',
                            'label' => 'TSIG secret',
                            'type' => 'password',
                            'store' => 'dns',
                            'config' => 'dns.key_secret',
                            'secure' => true,
                            'full' => true,
                            // Base64, because it is written into the key file's
                            // `secret "%s";` and a quote would close that clause early.
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['rfc2136']],
                            'rules' => ['required', 'string', 'max:255', 'regex:/^[A-Za-z0-9+\/=]+$/'],
                        ],
                        [
                            'key' => 'dns_cloudflare_token',
                            'label' => 'API token',
                            'type' => 'password',
                            'config' => 'dns.cloudflare.token',
                            'secure' => true,
                            'full' => true,
                            'helper' => 'Records are written DNS-only. A proxied record answers with Cloudflare\'s address, which hides the server and stops it taking a certificate.',
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['cloudflare']],
                            'rules' => ['required', 'string', 'max:255'],
                        ],
                        [
                            ...
                            'helper' => 'Resolved from the zone name when left empty.',
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['cloudflare']],
                            'rules' => ['nullable', 'string', 'max:64'],
                        ],
                        [
                            'key' => 'dns_hetzner_token',
                            'label' => 'API token',
                            'type' => 'password',
                            'config' => 'dns.hetzner.token',
                            'secure' => true,
                            'full' => true,
                            'helper' => 'From the Hetzner DNS console, not the Cloud project. The zone must be delegated to Hetzner\'s nameservers.',
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['hetzner']],
                            'rules' => ['required', 'string', 'max:255'],
                        ],
                        [
                            'key' => 'dns_hetzner_zone_id',
                            'label' => 'Zone ID',
                            'type' => 'text',
                            'config' => 'dns.hetzner.zone_id',
                            'helper' => 'Resolved from the zone name when left empty.',
                            'visible_when' => ['field' => 'dns_driver', 'is' => ['hetzner']],
                            'rules' => ['nullable', 'string', 'max:64'],
                        ],
