...
/*
 * Connection-limit key, for `limit_conn_zone` via `js_set`.
 *
 * $binary_remote_addr was the obvious key and the wrong one. A convention puts its
 * whole audience behind one NAT gateway, so every on-site viewer shared a single
 * bucket: a 10-connection cap meant ten people, and the eleventh got 503. The
 * playback token already identifies a viewer independently of where they are
 * sitting, so key on that instead.
 *
 * The claims are read without verifying the signature, which is safe for this
 * purpose: `verify` still runs in the access phase and rejects a forged token with
 * 403. Tampering with `sub` only moves the tamperer into a different bucket, which
 * is no better for them than having no limit at all, and they are not served either
 * way. Rate limiting runs in the preaccess phase, i.e. before that check, which is
 * why this cannot simply reuse the verified claims.
 *
 * An empty string disables the limit for that request, which nginx treats as "no
 * key". That is deliberate for internal traffic: the app's own playlist proxy and
 * ffmpeg thumbnail capture all arrive from one address and would otherwise share a
...
function viewerId(r) {
    try {
        ...
        if (token !== null) {
            ...
            if (parts.length === 3) {
                ...
                if (payload !== null) {
                    ...
                    // An embed key has no subject. The signature is the identity.
                    return 't:' + parts[2];
                }
            }
        }
        ...
        return 'a:' + r.variables.remote_addr;
    } catch (e) {
        // Never fail a request over a limiting key; fall back to the old behaviour.
        return 'a:' + r.variables.remote_addr;
    }
}

export default { verify, viewerId };
