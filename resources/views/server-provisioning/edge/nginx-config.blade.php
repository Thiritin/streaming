...
http {
    ...
    # Rate limiting
    # Connection limiting is keyed on the viewer, not the address.
    #
    # $binary_remote_addr put a whole venue behind one bucket: a convention NATs its
    # audience onto a single public IP, so a 10-connection cap meant ten people and
    # everyone after that got 503. hlsAuth.viewerId reads the playback token's
    # subject instead, and falls back to the address only for requests that carry no
    # token. See docker/edge-nginx/hls-auth.js.
    js_set $viewer_id hlsAuth.viewerId;
    limit_conn_zone $viewer_id zone=viewer_conn:10m;

    # limit_req is deliberately absent. At hls_time 2 a viewer makes about one
    # request a second, so the old 30r/s ceiling never shaped real load; all it did
    # was throttle everyone sharing a NAT. Connection count is the resource worth
    # bounding here, and auth already gates who gets served at all.
